...
def upload(request):
    """ Function to Upload a books from here """ 
    thank = False
    if request.method == "POST":
        ...
        if Books.objects.filter(book_name= book_name).exists():
            ...
            profile = Books(img=imgs, book_desc=book_desc, book_name=book_name, user = request.user)
            profile.save()
            messages.success(request, "Successfully Uploaded")
        thank = True
    return render(request, 'upload.html', {'thank' : thank})
